...
// Establish isolated testing directory for HOME and SHIPIT_HOME
...
// Set environment variables for isolation in current process
putenv("HOME={$tempHome}");
putenv("SHIPIT_HOME={$tempHome}");
$_ENV['HOME'] = $tempHome;
$_ENV['SHIPIT_HOME'] = $tempHome;

// Dynamically resolve a free TCP port using socket programming or fallback port probe
$port = 0;
...
echo "==================================================\n";
echo "ShipIt E2E Test Bootstrap Harness\n";
echo "==================================================\n";
echo "Temp Home Directory: {$tempHome}\n";
echo "Server URL:          {$serverUrl}\n";
echo "==================================================\n\n";

// Ensure the ui/public directory exists
...
// Setup PHP Server Process Pipes and Descriptors
...
if (!function_exists('recursiveRmdir')) {
...
register_shutdown_function(function () use (&$serverProcess, $tempHome, $stdoutFile, $stderrFile) {
    ...
    // Copy logs to agent folder before deleting
    $agentFolder = '/home/ian/Desktop/Packages/shipit/.agents/worker_m5_test';
    if (file_exists($stdoutFile)) {
        @copy($stdoutFile, $agentFolder . '/server_stdout.log');
    }
    if (file_exists($stderrFile)) {
        @copy($stderrFile, $agentFolder . '/server_stderr.log');
    }
...
// Perform readiness check on the server
